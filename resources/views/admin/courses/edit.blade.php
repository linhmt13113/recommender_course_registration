...
<div class="container mt-4">
    ...
    @endif
    <form action="{{ route('viewcourses.update', $course->course_id) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                @foreach($majors as $major)
                    ...
                    <div class="form-group border-bottom pb-2">
                        <div class="form-check">
                            ...
                            <input type="hidden" name="majors[{{ $major->major_id }}][major_id]" value="{{ $major->major_id }}">
                        ...
                        <div class="row mt-2">
                            <div class="col-md-6">
                                ...
                                <select name="majors[{{ $major->major_id }}][is_elective]" class="form-control">
                                    <option value="0" {{ ($courseMajorItem->pivot->is_elective ?? 0) == 0 ? 'selected' : '' }}>Mandatory</option>
        ...
        <div id="prerequisites_fields">
            ...
            <div class="prerequisite-group border p-2 mb-3">
                <div class="form-group">
                    <label>Select Applicable Major:</label>
                ...
                <div class="form-group">
                    ...
                    <select name="prerequisites[{{ $index }}][prerequisite_type]" class="form-control">
                        <option value="Required" {{ $prereq->prerequisite_type == 'Required' ? 'selected' : '' }}>Mandatory</option>
                        <option value="Optional" {{ $prereq->prerequisite_type == 'Optional' ? 'selected' : '' }}>Optional</option>
                        <option value="Previous" {{ $prereq->prerequisite_type == 'Previous' ? 'selected' : '' }}>Prior</option>
                    </select>
                </div>
            </div>
            @endforeach
